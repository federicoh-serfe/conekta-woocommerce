...
class WC_Conekta_Payment_Gateway extends WC_Conekta_Plugin {
	...
	public function ckpg_conekta_card_order_refunded( $order_id = null ) {
		...
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'woocommerce-process_checkout' ) ) {
			...
			\Conekta\Conekta::setLocale( 'es' );
			if ( ! $order_id ) {
				$order_id = isset( $_POST['order_id'] ) ? sanitize_text_field( wp_unslash( (string) $_POST['order_id'] ) ) : null;
			}
...
function ckpg_checkout_delete_card() {
	if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'woocommerce-process_checkout' ) ) {
		$payment_card_delete = isset( $_POST['value'] ) ? sanitize_text_field( wp_unslash( $_POST['value'] ) ) : null;
 ...
 * @param array    $data  Billing data array.
 */
function set_billing_data( $wc_order, $data ) {
	$wc_order->set_billing_address_1( isset( $data['address_1'] ) ? sanitize_text_field( wp_unslash( $data['address_1'] ) ) : '' );
	$wc_order->set_billing_address_2( isset( $data['address_2'] ) ? sanitize_text_field( wp_unslash( $data['address_2'] ) ) : '' );
	$wc_order->set_billing_city( isset( $data['city'] ) ? sanitize_text_field( wp_unslash( $data['city'] ) ) : '' );
	$wc_order->set_billing_state( isset( $data['state'] ) ? sanitize_text_field( wp_unslash( $data['state'] ) ) : '' );
	$wc_order->set_billing_country( isset( $data['country'] ) ? sanitize_text_field( wp_unslash( $data['country'] ) ) : '' );
	$wc_order->set_billing_email( isset( $data['email'] ) ? sanitize_text_field( wp_unslash( $data['email'] ) ) : '' );
	$wc_order->set_billing_first_name( isset( $data['firstName'] ) ? sanitize_text_field( wp_unslash( $data['firstName'] ) ) : '' );
	$wc_order->set_billing_last_name( isset( $data['lastName'] ) ? sanitize_text_field( wp_unslash( $data['lastName'] ) ) : '' );
	$wc_order->set_billing_phone( isset( $data['phone'] ) ? sanitize_text_field( wp_unslash( $data['phone'] ) ) : '' );
	$wc_order->set_billing_postcode( isset( $data['postcode'] ) ? sanitize_text_field( wp_unslash( $data['postcode'] ) ) : '' );
	$wc_order->save();
}
	...
	try {
		...
		if ( empty( $old_order ) ) {
			...
			if ( ! empty( $customer_id ) ) {
				...
				$customer_data = array(
					'name'  => ( isset( $_POST['firstName'] ) ? sanitize_text_field( wp_unslash( $_POST['firstName'] ) ) : '' ) . ' ' . ( isset( $_POST['lastName'] ) ? sanitize_text_field( wp_unslash( $_POST['lastName'] ) ) : '' ),
			...
			set_billing_data( $wc_order, $_POST );
			...
			$order_details = array(
				...
				'shipping_contact' => array(
					'phone'    => isset( $_POST['phone'] ) ? sanitize_text_field( wp_unslash( $_POST['phone'] ) ) : '',
					'receiver' => ( isset( $_POST['firstName'] ) ? sanitize_text_field( wp_unslash( $_POST['firstName'] ) ) : '' ) . ' ' . ( isset( $_POST['lastName'] ) ? sanitize_text_field( wp_unslash( $_POST['lastName'] ) ) : '' ),
					'address'  => array(
						'street1'     => isset( $_POST['address_1'] ) ? sanitize_text_field( wp_unslash( $_POST['address_1'] ) ) : '',
						'street2'     => isset( $_POST['address_2'] ) ? sanitize_text_field( wp_unslash( $_POST['address_2'] ) ) : '',
						'country'     => isset( $_POST['country'] ) ? sanitize_text_field( wp_unslash( $_POST['country'] ) ) : '',
						'postal_code' => isset( $_POST['postcode'] ) ? sanitize_text_field( wp_unslash( $_POST['postcode'] ) ) : '',
					),
				),
