...
?>
    ...
    <?php if ($this->enable_save_card) : ?>
        ...
        <input id="conekta-card-save"  type="checkbox" value="0" name="conekta-card-save" <?php echo ($this->enable_save_card)? 'checked': '' ?>> <?php echo esc_html($this->lang_options["enable_save_card"]); ?></input>
    </p>
    <?php endif; ?>
    </div>
<?php
?>
<div class = "customer-payment-sources" style="margin: 2rem; <?php echo ( !empty( $customer ) || !empty($sources))? '': 'display:none'?>" >
    <ul class="">
        <?php foreach( $customer->payment_sources as $info_card): ?>
            ...
            <li class="payment_method_2 card">
                <div style="border-bottom: 0.1rem solid #dedfdf;padding:2rem">
                    <a id=<?php echo "delete_payment_card_" .$info_card->id ?> value="<?php echo $info_card->id ?>"><img style="float:right;" src="<?php echo plugin_dir_url(__DIR__) . 'images/icons/trash-alt-solid.svg' ?>" alt="X" onclick="radiobutton('<?php echo $info_card->id ?>')"/></a>
                    <span class="spinner is-active" id="ckpg_checkout_delete_loader-gif" style="display:none"><img style="float:right;" src="/wp-admin/images/spinner.gif" alt="delete..."></span>
                    <input id="<?php echo "radio_input_" . $info_card->id ?>" type="radio" class="input-radio" style="float:left;" name="payment_card" value="<?php echo $info_card->id ?>" <?php echo ( $customer->default_payment_source_id == $info_card->id )? "checked" : "" ?>>
                    <label for="<?php echo esc_html("radio_input_" . $info_card->id) ?>"><strong><?php echo esc_html($info_card->customer['name'])?></strong>
                    </label>
    ...
    <?php if( !empty( $customer_id ) && !empty( $sources ) ) : ?>
        <a style="background-color:transparent;color:#cd2653; font-size: 1.7rem; width:100%; text-decoration:none" id="new_payment_card" onclick="ckpg_add_card()"><strong><?php echo esc_html( $this->lang_options["enter_card_details"]) ?></strong></a>
    <?php endif; ?>
    </ul>
</div>
<div class="clear"></div> 